...
module.exports = (client, member) => {
  console.log(member.lastMessage.member.guild.id);
  ...
  const chan = member.guild.channels.find(c => c.name === settings.goodbyeChannel);
  if (!chan) return;
  chan.send(goodbyeMessageFINAL).catch(console.error);
};
